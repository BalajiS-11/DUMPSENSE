...
export default function OfficerDashboard({ onSelectReport }) {
  ...
  const handleDownloadPdf = async (reportId) => {
    try {
      ...
      console.error('Failed to download PDF dossier:', err);
      setToastMessage({
        type: 'error',
        text: 'Failed to generate PDF dossier'
      });
      setTimeout(() => setToastMessage(null), 3500);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            ...
            <span>{t('officer_refresh')}</span>
          </button>
        </div>
      </div>

      {/* Tactical Surveillance Map (with Long Map Mode feature) */}
      <div className="bg-slate-900 rounded-card overflow-hidden border border-slate-700/80 shadow-xl transition-all duration-300">
        <div className="bg-slate-950 px-4 py-2.5 border-b border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-slate-400 hidden sm:inline">Live WebSockets Feed • Citywide</span>
            {/* Long Map feature toggle */}
            <button
              type="button"
              onClick={() => setIsFullMap(!isFullMap)}
              className={`px-2.5 py-1 rounded-element text-xs font-bold flex items-center gap-1.5 transition-colors ${
                isFullMap 
                  ? 'bg-accent text-white shadow-xs' 
                  : 'bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700'
              }`}
              title={isFullMap ? "Compress Map (Standard View)" : "Long Map Mode (Full View)"}
            >
              <span>{isFullMap ? "Standard Map" : "⛶ Long Map Mode"}</span>
            </button>
          </div>
        </div>
        <div className={`${isFullMap ? 'h-[750px] sm:h-[820px]' : 'h-[460px] sm:h-[500px]'} w-full relative transition-all duration-300`}>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-element border border-slate-200 text-xs">
              ...
              >
                ...
                <option value="unverified">{t('status_unverified')}</option>
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className="border-b border-slate-200 text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
                            <th className="py-3 px-4">{t('officer_col_evidence')}</th>
                            <th className="py-3 px-4">{t('officer_col_ai')}</th>
                            <th className="py-3 px-4">{t('officer_col_trust')}</th>
                            <th className="py-3 px-4">{t('officer_col_status')}</th>
                            <th className="py-3 px-4">{t('officer_col_logged')}</th>
                            <th className="py-3 px-4 text-right">{t('officer_col_action')}</th>
                          </tr>
                        </thead>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                    <th className="py-3.5 px-4">{t('officer_col_evidence')}</th>
                    <th className="py-3.5 px-4">{t('officer_col_ward')}</th>
                    <th className="py-3.5 px-4">{t('officer_col_ai')}</th>
                    <th className="py-3.5 px-4">{t('officer_col_trust')}</th>
                    <th className="py-3.5 px-4">{t('officer_col_status')}</th>
                    <th className="py-3.5 px-4">{t('officer_col_logged')}</th>
                    <th className="py-3.5 px-4 text-right">{t('officer_col_action')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
  ...
  function renderReportRow(report, showZone = false) {
    ...
    return (
      <tr key={report.id} className="hover:bg-slate-50/80 transition-colors group">
        ...
        {/* Actions (Inspect, PDF Download, Confirm, Reject) */}
        <td className="py-2.5 px-4 text-right whitespace-nowrap">
          <div className="inline-flex items-center gap-1.5">
            ...
            {/* Print / Save Official PDF Dossier (Fix 3) */}
            <button
