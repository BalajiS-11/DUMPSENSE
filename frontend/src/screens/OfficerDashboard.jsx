...
export default function OfficerDashboard({ onSelectReport }) {
  ...
  return (
    <div className="w-full min-h-screen bg-slate-100 flex flex-col">
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className={`fixed top-18 right-4 z-50 p-3 rounded-card text-xs font-semibold flex items-center gap-2 shadow-2xl border animate-fadeIn transition-all ${
      ...
      {/* FIX 6 Part A: Full-Page Tactical Surveillance Map */}
      {/* Height: calc(100vh - 64px) on desktop, 50vh on mobile */}
      <div className="w-full h-[50vh] md:h-[calc(100vh-64px)] relative bg-slate-950 border-b border-slate-800 shadow-2xl flex-shrink-0">
        <LiveMap onSelectReport={onSelectReport} />
      </div>

      {/* Incident Verification Table moves BELOW the map (User scrolls down to inspect) */}
      <div className="w-full max-w-app mx-auto px-4 sm:px-6 py-8 space-y-6 flex-1">
